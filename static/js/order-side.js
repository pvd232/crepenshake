...
const displayErrorMsg = (element) => {
  const selector = `#${
    element.closest(".card, .list-group-item").attr("id") + "error"
  }`;
  const id = `${
    element.closest(".card, .list-group-item").attr("id") + "error"
  }`;
...
const pageLogic = () => {
  ...
  $(".card-img-top").each(function () {
    ...
    if (selectedItemCategory == "ice_cream_bowl") {
      ...
      $(
        '<button class="btn" id="servingSize-2" type="button">Customize</button>'
      ).insertAfter($(this));
      $(
        '<button class="btn4" id="servingSize-1" type="button">Regular</button>'
      ).insertAfter($(this));
      $(
        '<button class="btn3" id="servingSize-0" type="button">Light</button>'
      ).insertAfter($(this));
      $('<button class="btn2" type="button">✓</button>').insertAfter($(this));
    }
  });
    ...
    .on("mouseenter", function () {
      if ($(this).attr("class") === "card") {
        ...
        if (selectedItemCategory != "toppings") {
          ...
          userOrderSide.checkIfIceCreamSelected()
        ) {
          $(this).find(".btn").show();
          $(this).find(".card-body").css("opacity", ".3");
          $(this).find(".card-img-top").css("opacity", ".3");
      ...
      $(this)
        .find(
          ".card-text, .card-title, .card-body, .card-img-top, .list-group-item"
        )
        .unbind("click")
        .bind("click", function (event) {
          ...
          if (
            ...
            const selectedItemCategory = $(this)
              .closest(".card-deck, .list-group")
              .attr("id");
            const json = JSON.parse(
              $(this).closest(".card, li").attr("data-sides")
            );
            ...
            if (selectedItemCategory != "toppings") {
              ...
              console.log("findSide", userOrderSide.findSide(json));
              if (!userOrderSide.findSide(json)) {
                ...
                if (
                  updatedSide.quantity === 3 &&
                  updatedSide.sideName === "ice_cream_bowl"
                ) {
      ...
      $(".btn")
        ...
        .bind("click", function () {
          const json = JSON.parse(
            $(this).closest(".card, .list-group-item").attr("data-sides")
          );
          const selectedItemCategory = $(this)
            .closest(".card-deck, .list-group")
            .attr("id");
      ...
      $(".btn6")
        ...
        .bind("click", function () {
          ...
          console.log("updated", updatedSide);
          if (updatedSide === 0) {
            $(this).closest(".card, .list-group-item").find(".btn2").hide();
...
const mobileRendering = () => {
  ...
  for (var i = 0; i < cardDeckElementsLength; i++) {
    ...
    listGroupTitle.setAttribute(
      "class",
      "col-12 col-sm-12 col-lg-12 col-md-12"
    );
    ...
    for (k; k < stoppingPoint; k++) {
      ...
      const data = JSON.parse(
        constCardData[counter].getAttribute("data-sides")
      );
      if (data.id) {
