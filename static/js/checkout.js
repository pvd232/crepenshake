...
function buildPage() {
  ...
  if (orderDict) {
    ...
    if (order) {
                ...
                $(
                  `<li class="list-group-item d-flex justify-content-between" id="crepeRow${i}${
                    j + 1
								...
								${
                  humanize(ingredient, "servingSize").servingSize +
                  " " +
                  humanize(ingredient, "id").id
                }</h5>
									</div><div class="col-4"><h4 style=''>$${ingredient.cost.toFixed(
                    2
                  )}</h4></div></li>`
                ...
                `<li class="list-group-item d-flex justify-content-between" id="crepeRow${i}${
                  j + 1
								...
								${menuCrepe.quantity + "x" + " " + humanize(menuCrepe, "name").name}</h5>
									</div><div class="col-4"><h4 style=''>$${menuCrepe.price.toFixed(
                    2
                  )}</h4></div></li>`
              ).insertAfter(`#crepeRow${i}${j}`);
            }
          ...
          for (var j = 0; j < drinks.length; j++) {
            ...
            if (drink instanceof Coffee) {
                ...
                $(
                  `<li class="list-group-item d-flex justify-content-between" id="checkoutDrinkRow${i}0">
                    ...
                    j + 1
									...
									<div class="col-8" style="margin-right: 0px; ">
										<h5 style=''>${
                      humanize(drink, "flavorSyrupServingSize")
              ...
              if (drink.drinkCategory === "milkshake") {
                ...
                if (drink.quantity > 1) {
                  drinkQuantity =
                    drink.quantity + "x" + " " + drink.servingSize;
                } else {
        ...
        for (var i = 0; i < orderSides.length; i++) {
          ...
          for (var j = 0; j < sideOrder.length; j++) {
            ...
            if (side.sideName === "ice_cream_bowl") {
              const lastElementId = $("#checkingCartBody0")
                .find("li")
                .last()
                .attr("id");
              $(
                  ...
                  $(
                    `<li class="list-group-item d-flex justify-content-between" id="checkoutSideRow${i}${
                      j + 1
								...
								${
                  humanize(topping, "servingSize").servingSize +
                  " " +
                  humanize(topping, "id").id
                }</h5>
									</div><div class="col-4"><h4 style=''>$${topping.cost.toFixed(
                    2
                  )}</h4></div></li>`
                  ).insertAfter(`#${lastElementId}`);
                }
      ...
      $(`#checkoutFooter`)
        .append(`<div class="col-8"><h4 style="font-weight: bold;">Order Total</h4>
    ...
    .then(function (data) {
      ...
      card.on("change", function (event) {
        ...
        if (event.error) {
          ...
          document.querySelector("#cardError").textContent = event.error
            ? event.error.message
            : "";
          $("#cardError").show();
