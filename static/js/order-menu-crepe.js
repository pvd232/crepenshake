...
const checkOut = (orderCrepe) => {
  ...
  if (editCrepeIndex != null) {
    ...
    $.when(stringify(orderCrepe)).then(
  ...
  $(".card, .list-group")
    .on("mouseenter", function () {
        ...
        .bind("click", function (event) {
          ...
          if (
            ...
            const json = JSON.parse(
              $(this).closest(".card, li").attr("data-menucrepes")
            );
      ...
      $(".btn6")
        ...
        .bind("click", function () {
          const json = JSON.parse(
            $(this).closest(".card, .list-group-item").attr("data-menucrepes")
          );
          const selectedCrepe = userOrderMenuCrepe.findCrepe(json);
  ...
  for (var i = 0; i < cardDeckElementsLength; i++) {
    ...
    for (k; k < stoppingPoint; k++) {
      ...
      const data = JSON.parse(
        constCardData[counter].getAttribute("data-menucrepes")
      );
