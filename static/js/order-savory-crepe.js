  ...
  $(".card, .list-group-item")
    .on("mouseenter", function () {
      if (
        ...
        $(this)
          .find(".card-body, .card-img-top, img, h5, p")
          .css("opacity", ".3");
        $(this).find(".btn").show();
      }
            ...
            for (var i = 0; i < userOrderCrepe.ingredients.length; i++) {
              ...
              if (ingredient.category === "protein") {
                if (ingredient.servingSize === "light") {
                  ...
                  userOrderCrepe.ingredients[i].servingSize === "regular"
                ) {
                  $(`#${ingredient.id}`).find(".btn2").html("✓");
                } else if (
                  userOrderCrepe.ingredients[i].servingSize === "extra"
                ) {
                  $(`#${ingredient.id}`).find(".btn2").html("2x");
                }
              }
      ...
      $(".btn")
        ...
        .bind("click", function () {
          const json = JSON.parse(
            $(this).closest(".card, .list-group-item").attr("data-ingredients")
          );
